...
import teamcerberus.cerberuscore.util.SynchronizedWeakIdentityListMap;
 ...
 * System-owned parts have subhit = (-1 - partNumber) and are handled by the IMicroblockCoverSystem if available.
 * Tile-owned parts have subhit = partNumber and are handled by the IMultipartTile. 
 * 
 * If all tile-owned parts are broken the tile should call IMicroblockCoverSystem.convertToSystemBlock to convert
...
public abstract class BlockMicroblockBase extends BlockContainer {
	...
	public void addCollisionBoxesToList(World world, int x, int y, int z,
		...
		try {
			IMicroblockSupporterTile te = ((IMicroblockSupporterTile) world.getBlockTileEntity(x,
					y, z));
			MicroblockCoverSystem ci = te.getCoverSystem();
			if (ci != null)
	...
	public void harvestBlockMultipart(World world, EntityPlayer ply, int x,
			int y, int z, int blockMetadata) {
	...
	@Override
	public boolean removeBlockByPlayer(World w, EntityPlayer ply, int x, int y,
			int z) {
		return removeBlockByPlayerStatic(w, ply, x, y, z);
	}

	public static boolean removeBlockByPlayerStatic(World w, EntityPlayer ply,
			int x, int y, int z) {
		...
		if (ply.worldObj.isRemote) {
			...
			return true; // on the client, do nothing (let the server handle
							// multipart breaking)
		...
		if (!coord.isCoverSystemPart) {
			lastDrop = ((IMicroblockSupporterTile) te)
					.removePartByPlayer(ply, coord.part);
		} else {
			lastDrop = ((IMicroblockSupporterTile) te).getCoverSystem()
					.removePartByPlayer(ply, coord.part);
		}
	...
	private static void sendDigStart() {
		...
		if (coord != null)
			MultiblockPacketHandler.instance.sendDigStart(coord);
		// APILocator.getNetManager().sendToServer(new
		// PacketMultipartDigStart(coord));
	}
	...
	private static boolean updateBreakingPart(int x, int y, int z) {
		...
		MovingObjectPosition ray = ply.rayTrace(
				Minecraft.getMinecraft().playerController
						.getBlockReachDistance(), 0);
		PartCoordinates _new = null;
		if (ray == null || ray.typeOfHit != EnumMovingObjectType.TILE
				|| ray.blockX != x || ray.blockY != y || ray.blockZ != z) {
			breaking_part.remove(ply);

		} else {
			if (ray.subHit >= 0)
				breaking_part.put(ply, _new = new PartCoordinates(x, y, z,
						ray.subHit, false));
			else
				breaking_part.put(ply, _new = new PartCoordinates(x, y, z, -1
						- ray.subHit, true));
		}

		boolean changed = (old == null && _new != null)
				|| (old != null && !old.equals(_new));
	...
	@Override
	public final float getPlayerRelativeBlockHardness(EntityPlayer ply,
			World world, int x, int y, int z) {
		return getPlayerRelativeBlockHardnessStatic(ply, world, x, y, z);
	}

	public static float getPlayerRelativeBlockHardnessStatic(EntityPlayer ply,
			World world, int x, int y, int z) {
		...
		if (te == null || !(te instanceof IMicroblockSupporterTile)) {
			return 0.01f;
		}

		else if (!part.isCoverSystemPart) {
			return ((IMicroblockSupporterTile) te).getPlayerRelativePartHardness(ply,
					part.part);

		} else {
			MicroblockCoverSystem ci = ((IMicroblockSupporterTile) te).getCoverSystem();
			return ci == null ? -1 : ci.getPlayerRelativePartHardness(ply,
					part.part);
		}
	}
	...
	@Override
	public final ArrayList<ItemStack> getBlockDropped(World world, int x,
	...
	protected BlockMicroblockBase(int id, Material mat) {
		super(id, mat);
	}
	 ...
	 */
	public MovingObjectPosition wrappedCollisionRayTrace(World world, int i,
			int j, int k, Vec3 vec3d, Vec3 vec3d1) {
		return null;
	}

	@Override
	public final MovingObjectPosition collisionRayTrace(World world, int x,
			int y, int z, Vec3 src, Vec3 dst) {
		try {
			IMicroblockSupporterTile tile = (IMicroblockSupporterTile) world.getBlockTileEntity(x,
					y, z);

			MicroblockCoverSystem ci = tile.getCoverSystem();
			MovingObjectPosition ciPos = ci == null ? null : ci
					.collisionRayTrace(src, dst);

			MovingObjectPosition tilePos = wrappedCollisionRayTrace(world, x,
					y, z, src, dst);

			if (tilePos != null && tilePos.subHit < 0)
				throw new AssertionError(
						"wrappedCollisionRayTrace must return a non-negative subHit");
			...
			if (tilePos != null && tilePos.subHit < 0)
				throw new AssertionError(
						"ICoverableTile.collisionRayTrace must return a non-negative subHit");
	...
	public final int getRenderType() {
		return useWrappedRenderType ? wrappedGetRenderType()
				: MultiblockManager.renderType;
	}
	...
	public static void setBreakingPart(EntityPlayer source, PartCoordinates part) {
		...
		if (source.capabilities.isCreativeMode) {
			removeBlockByPlayerStatic(source.worldObj, source, part.x, part.y,
					part.z);
		}

		// for(EntityPlayer pl :
		// (List<EntityPlayer>)source.worldObj.playerEntities)
		// APILocator.getNetManager().sendToClient(new
		// PacketUpdateBreakingPart(part), pl);
	}

	@Override
	public final int getDamageValue(World par1World, int par2, int par3,
			int par4) {
	...
	@Override
	public final ItemStack getPickBlock(MovingObjectPosition target,
			World world, int x, int y, int z) {
		IMicroblockSupporterTile te = (IMicroblockSupporterTile) world.getBlockTileEntity(x, y, z);
		if (te == null)
			return null;
	...
	public boolean isBlockSolidOnSide(World world, int x, int y, int z,
			ForgeDirection side) {
		IMicroblockSupporterTile imt = ((IMicroblockSupporterTile) world
	...
	public static void renderInvBlockStatic(RenderBlocks render, Block block,
			int meta) {
		useWrappedRenderType = true;
		/*
		 * String tex = block.getTextureFile(); if(!tex.equals("/terrain.png"))
		 * ForgeHooksClient.bindTexture(tex, 0);
		 */
		render.renderBlockAsItem(block, meta, 1);
		/*
		 * if(!tex.equals("/terrain.png")) ForgeHooksClient.unbindTexture();
		 */
		useWrappedRenderType = false;
	}

	public static void renderBlockStatic(RenderBlocks render, Block block,
			int x, int y, int z) {
		useWrappedRenderType = true;
		/*
		 * String tex = block.getTextureFile();
		 * 
		 * boolean useTexture = render.overrideBlockTexture != null &&
		 * !tex.equals("/terrain.png");
		 * 
		 * if(useTexture) ForgeHooksClient.bindTexture(tex, 0);
		 */
		render.renderBlockByRenderType(block, x, y, z);
		/*
		 * if(useTexture) ForgeHooksClient.unbindTexture();
		 */
		useWrappedRenderType = false;
	}
