...
import teamcerberus.cerberuscore.microblock.part.EnumAxisPosition;
...
public class MicroblockPlacementHighlightHandler {
	...
	public void onHighlight(DrawBlockHighlightEvent evt) {
		...
		if(holding != null && holding.itemID == MultiblockManager.microblockContainerBlock.blockID)
		{
			...
			if(type != null)
			{
				...
				switch(pos.sideHit)
				{
				case EnumPosition.NX: if(hitPos == null || hitPos.x.touchesNegative()) dx = -1; break;
				case EnumPosition.PX: if(hitPos == null || hitPos.x.touchesPositive()) dx = 1; break;
				case EnumPosition.NY: if(hitPos == null || hitPos.y.touchesNegative()) dy = -1; break;
				case EnumPosition.PY: if(hitPos == null || hitPos.y.touchesPositive()) dy = 1; break;
				case EnumPosition.NZ: if(hitPos == null || hitPos.z.touchesNegative()) dz = -1; break;
				case EnumPosition.PZ: if(hitPos == null || hitPos.z.touchesPositive()) dz = 1; break;
				}
				...
				if(ok)
				{
					...
					if(hitPos != null) {
						switch(pos.sideHit)
						{
						case EnumPosition.NX: inset =     hitBB.minX; break;
						case EnumPosition.PX: inset = 1 - hitBB.maxX; break;
						case EnumPosition.NY: inset =     hitBB.minY; break;
						case EnumPosition.PY: inset = 1 - hitBB.maxY; break;
						case EnumPosition.NZ: inset =     hitBB.minZ; break;
						case EnumPosition.PZ: inset = 1 - hitBB.maxZ; break;
						}
						inset -= 0.01;
					}
	...
	public static EnumPosition getPanelPlacement(EntityPlayer ply, MovingObjectPosition pos, EnumPosition placingOn) {
		...
		switch(pos.sideHit)
		{
		case EnumPosition.NX:
		case EnumPosition.PX:
			if(y >= -BORDER && y <= BORDER && z >= -BORDER && z <= BORDER)
				result = (pos.sideHit == EnumPosition.NX ? EnumPosition.FacePX : EnumPosition.FaceNX);
			else if(Math.abs(y) < Math.abs(z))
				result = (z < 0 ? EnumPosition.FaceNZ : EnumPosition.FacePZ);
			else
				result = (y < 0 ? EnumPosition.FaceNY : EnumPosition.FacePY);
			break;
		case EnumPosition.NY:
		case EnumPosition.PY:
			if(x >= -BORDER && x <= BORDER && z >= -BORDER && z <= BORDER)
				result = (pos.sideHit == EnumPosition.NY ? EnumPosition.FacePY : EnumPosition.FaceNY);
			else if(Math.abs(z) < Math.abs(x))
				result = (x < 0 ? EnumPosition.FaceNX : EnumPosition.FacePX);
			else
				result = (z < 0 ? EnumPosition.FaceNZ : EnumPosition.FacePZ);
			break;
		case EnumPosition.NZ:
		case EnumPosition.PZ:
			if(x >= -BORDER && x <= BORDER && y >= -BORDER && y <= BORDER)
				result = (pos.sideHit == EnumPosition.NZ ? EnumPosition.FacePZ : EnumPosition.FaceNZ);
		...
		if(placingOn != null)
		{
			switch(result)
			{
			case FaceNX: if(pos.sideHit == EnumPosition.PX && (placingOn.x == EnumAxisPosition.Negative || placingOn.x == EnumAxisPosition.Centre)) result = EnumPosition.FacePX; break;
			case FaceNY: if(pos.sideHit == EnumPosition.PY && (placingOn.y == EnumAxisPosition.Negative || placingOn.y == EnumAxisPosition.Centre)) result = EnumPosition.FacePY; break;
			case FaceNZ: if(pos.sideHit == EnumPosition.PZ && (placingOn.z == EnumAxisPosition.Negative || placingOn.z == EnumAxisPosition.Centre)) result = EnumPosition.FacePZ; break;
			case FacePX: if(pos.sideHit == EnumPosition.NX && (placingOn.x == EnumAxisPosition.Positive || placingOn.x == EnumAxisPosition.Centre)) result = EnumPosition.FaceNX; break;
			case FacePY: if(pos.sideHit == EnumPosition.NY && (placingOn.y == EnumAxisPosition.Positive || placingOn.y == EnumAxisPosition.Centre)) result = EnumPosition.FaceNY; break;
			case FacePZ: if(pos.sideHit == EnumPosition.NZ && (placingOn.z == EnumAxisPosition.Positive || placingOn.z == EnumAxisPosition.Centre)) result = EnumPosition.FaceNZ; break;
	...
	public static EnumPosition getStripPlacement(EntityPlayer ply, MovingObjectPosition pos, EnumPosition placingOn) {
		Vec3 hv = pos.hitVec.addVector(-pos.blockX, -pos.blockY, -pos.blockZ);
		double x = hv.xCoord - 0.5;
		double y = hv.yCoord - 0.5;
		double z = hv.zCoord - 0.5;
	...
	public static EnumPosition getCornerPlacement(EntityPlayer ply, MovingObjectPosition pos, EnumPosition placingOn) {
		Vec3 hv = pos.hitVec.addVector(-pos.blockX, -pos.blockY, -pos.blockZ);
		double x = hv.xCoord - 0.5;
		double y = hv.yCoord - 0.5;
		double z = hv.zCoord - 0.5;
	...
	private static void renderEdgeSelector(MovingObjectPosition pos, EntityPlayer ply, double outer_size, double inset)
	{
		...
		case EnumPosition.NX:
		case EnumPosition.PX:
			double x = (pos.sideHit == EnumPosition.NX ? inset : 1 - inset);
			t.addVertex(x, 0, 0);
			t.addVertex(x, 0, 1);
			t.addVertex(x, 0, 1);
			t.addVertex(x, 1, 1);
		...
		case EnumPosition.NY:
		case EnumPosition.PY:
			double y = (pos.sideHit == EnumPosition.NY ? inset : 1 - inset);
			t.addVertex(0, y, 0);
			t.addVertex(0, y, 1);
			t.addVertex(0, y, 1);
			t.addVertex(1, y, 1);
		...
		case EnumPosition.NZ:
		case EnumPosition.PZ:
			double z = (pos.sideHit == EnumPosition.NZ ? inset : 1 - inset);
			t.addVertex(0, 0, z);
			t.addVertex(0, 1, z);
			t.addVertex(0, 1, z);
			t.addVertex(1, 1, z);
