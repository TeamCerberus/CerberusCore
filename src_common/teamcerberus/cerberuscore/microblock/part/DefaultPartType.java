...
public class DefaultPartType implements PartType<Part> {
	...
	public ItemStack getDroppedStack(Part part, EntityPlayer ply) {
		return MicroblockSystem.partTypeIDToItemStack(
				id, 1);
	}
	...
	public float getPlayerRelativeHardness(Part part, EntityPlayer ply) {
		...
		return ply.getCurrentPlayerStrVsBlock(modelBlock, false, modelMeta)
				/ hardness / 30F;
	}
	...
	public DefaultPartType(int id, EnumPartClass clazz, double size, String unlocalizedBlockName,
			Block modelBlock, int modelMeta) {
		this.clazz = clazz;
		this.size = size;
		this.id = id;
	...
	@Override
	@SideOnly(Side.CLIENT)
	public void renderPreview(RenderGlobal context, EnumPosition pos,
			ItemStack stack) {
...
//				.func_110577_a(TextureMap.field_110575_b);//TODO: DOES SOMETHIGN?
	...
	private void vertXY(Tessellator t, double x, double y, double z) {
		t.addVertexWithUV(x, y, z, uMin + (uMax - uMin) * x, vMax
				+ (vMin - vMax) * y);
	}

	@SideOnly(Side.CLIENT)
	private void vertXZ(Tessellator t, double x, double y, double z) {
		t.addVertexWithUV(x, y, z, uMin + (uMax - uMin) * x, vMin
				+ (vMax - vMin) * z);
	}

	@SideOnly(Side.CLIENT)
	private void vertYZ(Tessellator t, double x, double y, double z) {
		t.addVertexWithUV(x, y, z, uMin + (uMax - uMin) * z, vMax
				+ (vMin - vMax) * y);
	}

	@SideOnly(Side.CLIENT)
	private void renderHollowPanelYZ(Tessellator t, AxisAlignedBB aabb) {
		setTex(modelBlock.getIcon(EnumPosition.NX, modelMeta));
		t.setNormal(-1, 0, 0);
		...
		setTex(modelBlock.getIcon(EnumPosition.PX, modelMeta));
		t.setNormal(1, 0, 0);
		...
		setTex(modelBlock.getIcon(EnumPosition.NY, modelMeta));
		t.setNormal(0, -1, 0);
		...
		setTex(modelBlock.getIcon(EnumPosition.PY, modelMeta));
		t.setNormal(0, 1, 0);
		...
		setTex(modelBlock.getIcon(EnumPosition.NZ, modelMeta));
		t.setNormal(0, 0, -1);
		...
		setTex(modelBlock.getIcon(EnumPosition.PZ, modelMeta));
		t.setNormal(0, 0, 1);
	...
	private static int rgb(double r, double g, double b) {
		return (rgb_component(r) << 16) | (rgb_component(g) << 8)
				| rgb_component(b);
	}
	...
	private static int interpRGB(int a, int b, double i) {
		return rgb(interp(rgb_r(a), rgb_r(b), i) / 255.0,
				interp(rgb_g(a), rgb_g(b), i) / 255.0,
				interp(rgb_b(a), rgb_b(b), i) / 255.0);
	}
	...
	private void setColourAndBrightness(Tessellator t, double x, double y) {
		int col = interpRGB(interpRGB(colNN, colPN, x),
				interpRGB(colNP, colPP, x), y);
		int bri = interpBrightness(interpBrightness(briNN, briPN, x),
				interpBrightness(briNP, briPP, x), y);
		t.setColorOpaque_I(col);
		t.setBrightness(bri);
	}

	@SideOnly(Side.CLIENT)
	private void getBrightnessXFace(RenderBlocks rb, int bx, int by, int bz,
			double f_dx) {
		...
		if (rb == null) {
			...
			bx += dx;
			briNN = briNP = briPN = briPP = rb.blockAccess
					.getLightBrightnessForSkyBlocks(bx, by, bz, 0);
		}
	}

	@SideOnly(Side.CLIENT)
	private void getBrightnessYFace(RenderBlocks rb, int bx, int by, int bz,
			double f_dy) {
	...
	@SideOnly(Side.CLIENT)
	private void getBrightnessZFace(RenderBlocks rb, int bx, int by, int bz,
			double f_dz) {
	...
	@SideOnly(Side.CLIENT)
	private void renderAABB(Tessellator t, AxisAlignedBB aabb, RenderBlocks rb,
			int bx, int by, int bz, Block par1Block, boolean[] dontRenderFaces) {
		...
		if (tzmin > DRFMIN || !dontRenderFaces[EnumPosition.NZ]) {
			setTex(EnumPosition.NZ);
			t.setNormal(0, 0, -1);
			colNN = colPN = colNP = colPP = rgb(colourR * 0.8, colourG * 0.8,
					colourB * 0.8);
			getBrightnessZFace(rb, bx, by, bz, rbb.minZ);
			setColourAndBrightness(t, rbb.minX, rbb.minY);
			t.addVertexWithUV(aabb.minX, aabb.minY, aabb.minZ, uMax + txmin
					* (uMin - uMax), vMax + tymin * (vMin - vMax));
			setColourAndBrightness(t, rbb.minX, rbb.maxY);
			t.addVertexWithUV(aabb.minX, aabb.maxY, aabb.minZ, uMax + txmin
					* (uMin - uMax), vMax + tymax * (vMin - vMax));
			setColourAndBrightness(t, rbb.maxX, rbb.maxY);
			t.addVertexWithUV(aabb.maxX, aabb.maxY, aabb.minZ, uMax + txmax
					* (uMin - uMax), vMax + tymax * (vMin - vMax));
			setColourAndBrightness(t, rbb.maxX, rbb.minY);
			t.addVertexWithUV(aabb.maxX, aabb.minY, aabb.minZ, uMax + txmax
					* (uMin - uMax), vMax + tymin * (vMin - vMax));
		}

		if (tymin > DRFMIN || !dontRenderFaces[EnumPosition.NY]) {
			setTex(EnumPosition.NY);
			colNN = colPN = colNP = colPP = rgb(colourR * 0.5, colourG * 0.5,
					colourB * 0.5);
			t.setNormal(0, -1, 0);
			getBrightnessYFace(rb, bx, by, bz, rbb.minY);
			setColourAndBrightness(t, rbb.maxX, rbb.minZ);
			t.addVertexWithUV(aabb.maxX, aabb.minY, aabb.minZ, uMin + txmax
					* (uMax - uMin), vMin + tzmin * (vMax - vMin));
			setColourAndBrightness(t, rbb.maxX, rbb.maxZ);
			t.addVertexWithUV(aabb.maxX, aabb.minY, aabb.maxZ, uMin + txmax
					* (uMax - uMin), vMin + tzmax * (vMax - vMin));
			setColourAndBrightness(t, rbb.minX, rbb.maxZ);
			t.addVertexWithUV(aabb.minX, aabb.minY, aabb.maxZ, uMin + txmin
					* (uMax - uMin), vMin + tzmax * (vMax - vMin));
			setColourAndBrightness(t, rbb.minX, rbb.minZ);
			t.addVertexWithUV(aabb.minX, aabb.minY, aabb.minZ, uMin + txmin
					* (uMax - uMin), vMin + tzmin * (vMax - vMin));
		}

		if (txmin > DRFMIN || !dontRenderFaces[EnumPosition.NX]) {
			setTex(EnumPosition.NX);
			t.setNormal(-1, 0, 0);
			colNN = colPN = colNP = colPP = rgb(colourR * 0.6, colourG * 0.6,
					colourB * 0.6);
			getBrightnessXFace(rb, bx, by, bz, rbb.minX);
			setColourAndBrightness(t, rbb.minY, rbb.minZ);
			t.addVertexWithUV(aabb.minX, aabb.minY, aabb.minZ, uMin + tzmin
					* (uMax - uMin), vMax + tymin * (vMin - vMax));
			setColourAndBrightness(t, rbb.minY, rbb.maxZ);
			t.addVertexWithUV(aabb.minX, aabb.minY, aabb.maxZ, uMin + tzmax
					* (uMax - uMin), vMax + tymin * (vMin - vMax));
			setColourAndBrightness(t, rbb.maxY, rbb.maxZ);
			t.addVertexWithUV(aabb.minX, aabb.maxY, aabb.maxZ, uMin + tzmax
					* (uMax - uMin), vMax + tymax * (vMin - vMax));
			setColourAndBrightness(t, rbb.maxY, rbb.minZ);
			t.addVertexWithUV(aabb.minX, aabb.maxY, aabb.minZ, uMin + tzmin
					* (uMax - uMin), vMax + tymax * (vMin - vMax));
		}

		if (tzmax < DRFMAX || !dontRenderFaces[EnumPosition.PZ]) {
			setTex(EnumPosition.PZ);
			t.setNormal(0, 0, 1);
			colNN = colPN = colNP = colPP = rgb(colourR * 0.8, colourG * 0.8,
					colourB * 0.8);
			...
			t.addVertexWithUV(aabb.maxX, aabb.minY, aabb.maxZ, uMin + txmax
					* (uMax - uMin), vMax + tymin * (vMin - vMax));
			setColourAndBrightness(t, rbb.maxX, rbb.maxY);
			t.addVertexWithUV(aabb.maxX, aabb.maxY, aabb.maxZ, uMin + txmax
					* (uMax - uMin), vMax + tymax * (vMin - vMax));
			setColourAndBrightness(t, rbb.minX, rbb.maxY);
			t.addVertexWithUV(aabb.minX, aabb.maxY, aabb.maxZ, uMin + txmin
					* (uMax - uMin), vMax + tymax * (vMin - vMax));
			setColourAndBrightness(t, rbb.minX, rbb.minY);
			t.addVertexWithUV(aabb.minX, aabb.minY, aabb.maxZ, uMin + txmin
					* (uMax - uMin), vMax + tymin * (vMin - vMax));
		...
		if (tymax < DRFMAX || !dontRenderFaces[EnumPosition.PY]) {
			setTex(EnumPosition.PY);
			t.setNormal(0, 1, 0);
			colNN = colPN = colNP = colPP = rgb(colourR * 1.0, colourG * 1.0,
					colourB * 1.0);
			getBrightnessYFace(rb, bx, by, bz, rbb.maxY);
			setColourAndBrightness(t, rbb.minX, rbb.minZ);
			t.addVertexWithUV(aabb.minX, aabb.maxY, aabb.minZ, uMin + txmin
					* (uMax - uMin), vMin + tzmin * (vMax - vMin));
			setColourAndBrightness(t, rbb.minX, rbb.maxZ);
			t.addVertexWithUV(aabb.minX, aabb.maxY, aabb.maxZ, uMin + txmin
					* (uMax - uMin), vMin + tzmax * (vMax - vMin));
			...
			t.addVertexWithUV(aabb.maxX, aabb.maxY, aabb.maxZ, uMin + txmax
					* (uMax - uMin), vMin + tzmax * (vMax - vMin));
			setColourAndBrightness(t, rbb.maxX, rbb.minZ);
			t.addVertexWithUV(aabb.maxX, aabb.maxY, aabb.minZ, uMin + txmax
					* (uMax - uMin), vMin + tzmin * (vMax - vMin));
		}

		if (txmax < DRFMAX || !dontRenderFaces[EnumPosition.PX]) {
			setTex(EnumPosition.PX);
			t.setNormal(1, 0, 0);
			colNN = colPN = colNP = colPP = rgb(colourR * 0.6, colourG * 0.6,
					colourB * 0.6);
			...
			setColourAndBrightness(t, rbb.maxY, rbb.minZ);
			t.addVertexWithUV(aabb.maxX, aabb.maxY, aabb.minZ, uMax + tzmin
					* (uMin - uMax), vMax + tymax * (vMin - vMax));
			setColourAndBrightness(t, rbb.maxY, rbb.maxZ);
			t.addVertexWithUV(aabb.maxX, aabb.maxY, aabb.maxZ, uMax + tzmax
					* (uMin - uMax), vMax + tymax * (vMin - vMax));
			setColourAndBrightness(t, rbb.minY, rbb.maxZ);
			t.addVertexWithUV(aabb.maxX, aabb.minY, aabb.maxZ, uMax + tzmax
					* (uMin - uMax), vMax + tymin * (vMin - vMax));
			setColourAndBrightness(t, rbb.minY, rbb.minZ);
			t.addVertexWithUV(aabb.maxX, aabb.minY, aabb.minZ, uMax + tzmin
					* (uMin - uMax), vMax + tymin * (vMin - vMax));
		}
	}
	...
	public void renderPartInv(RenderBlocks render, ItemStack stack) {
		...
		case Strip:
			...
			GL11.glTranslatef(0.5f - (float) size / 2, 0,
					0.5f - (float) size / 2);
			break;
		case Corner:
			pos = EnumPosition.CornerNXNYNZ;
			GL11.glTranslatef(0.5f - (float) size / 2, 0.5f - (float) size / 2,
					0.5f - (float) size / 2);
			break;
		default:
			pos = EnumPosition.Centre;
...
//				.func_110577_a(TextureMap.field_110575_b);//TODO: DOES SOMETHIGN?
	...
	private void renderAABB(RenderBlocks render, int x, int y, int z,
			double minx, double miny, double minz, double maxx, double maxy,
			double maxz, Block block, boolean[] dontRenderFaces) {
		renderAABB(
				Tessellator.instance,
				AxisAlignedBB.getAABBPool()
						.getAABB(minx, miny, minz, maxx, maxy, maxz)
						.offset(x, y, z), render, x, y, z, block,
				dontRenderFaces);
	}

	@Override
	@SideOnly(Side.CLIENT)
	public void renderPartWorld(RenderBlocks render, Part p, int x, int y,
			int z, boolean[] dontRenderFaces) {
		...
		if (clazz == EnumPartClass.HollowPanel) {
			...
			switch (p.pos) {
			case FaceNX:
				renderAABB(render, x, y, z, i, i, i, s, hes, 1 - i, modelBlock,
						dontRenderFaces);
				renderAABB(render, x, y, z, i, 1 - hes, i, s, 1 - i, 1 - i,
						modelBlock, dontRenderFaces);
				renderAABB(render, x, y, z, i, hes, i, s, 1 - hes, hes,
						modelBlock, dontRenderFaces);
				renderAABB(render, x, y, z, i, hes, 1 - hes, s, 1 - hes, 1 - i,
						modelBlock, dontRenderFaces);
				break;
			case FacePX:
				renderAABB(render, x, y, z, 1 - s, i, i, 1 - i, hes, 1 - i,
						modelBlock, dontRenderFaces);
				renderAABB(render, x, y, z, 1 - s, 1 - hes, i, 1 - i, 1 - i,
						1 - i, modelBlock, dontRenderFaces);
				renderAABB(render, x, y, z, 1 - s, hes, i, 1 - i, 1 - hes, hes,
						modelBlock, dontRenderFaces);
				renderAABB(render, x, y, z, 1 - s, hes, 1 - hes, 1 - i,
						1 - hes, 1 - i, modelBlock, dontRenderFaces);
				break;
			case FaceNY:
				renderAABB(render, x, y, z, i, i, i, hes, s, 1 - i, modelBlock,
						dontRenderFaces);
				renderAABB(render, x, y, z, 1 - hes, i, i, 1 - i, s, 1 - i,
						modelBlock, dontRenderFaces);
				renderAABB(render, x, y, z, hes, i, i, 1 - hes, s, hes,
						modelBlock, dontRenderFaces);
				renderAABB(render, x, y, z, hes, i, 1 - hes, 1 - hes, s, 1 - i,
						modelBlock, dontRenderFaces);
				break;
			case FacePY:
				renderAABB(render, x, y, z, i, 1 - s, i, hes, 1 - i, 1 - i,
						modelBlock, dontRenderFaces);
				renderAABB(render, x, y, z, 1 - hes, 1 - s, i, 1 - i, 1 - i,
						1 - i, modelBlock, dontRenderFaces);
				renderAABB(render, x, y, z, hes, 1 - s, i, 1 - hes, 1 - i, hes,
						modelBlock, dontRenderFaces);
				renderAABB(render, x, y, z, hes, 1 - s, 1 - hes, 1 - hes,
						1 - i, 1 - i, modelBlock, dontRenderFaces);
				break;
			case FaceNZ:
				renderAABB(render, x, y, z, i, i, i, hes, 1 - i, s, modelBlock,
						dontRenderFaces);
				renderAABB(render, x, y, z, 1 - hes, i, i, 1 - i, 1 - i, s,
						modelBlock, dontRenderFaces);
				renderAABB(render, x, y, z, hes, i, i, 1 - hes, hes, s,
						modelBlock, dontRenderFaces);
				renderAABB(render, x, y, z, hes, 1 - hes, 0, 1 - hes, 1 - i, s,
						modelBlock, dontRenderFaces);
				break;
			case FacePZ:
				renderAABB(render, x, y, z, i, i, 1 - s, hes, 1 - i, 1 - i,
						modelBlock, dontRenderFaces);
				renderAABB(render, x, y, z, 1 - hes, i, 1 - s, 1 - i, 1 - i,
						1 - i, modelBlock, dontRenderFaces);
				renderAABB(render, x, y, z, hes, i, 1 - s, 1 - hes, hes, 1 - i,
						modelBlock, dontRenderFaces);
				renderAABB(render, x, y, z, hes, 1 - hes, 1 - s, 1 - hes,
						1 - i, 1 - i, modelBlock, dontRenderFaces);
				break;
			default:
				// shouldn't happen
				System.err.println("hollow panel placed at invalid position "
						+ p.pos + " in block " + x + "," + y + "," + z);
			}
		} else {
			...
			if (bb.maxZ == 1)
				bb.maxZ -= i;
			renderAABB(render, x, y, z, bb.minX, bb.minY, bb.minZ, bb.maxX,
					bb.maxY, bb.maxZ, modelBlock, dontRenderFaces);
		}
	}
